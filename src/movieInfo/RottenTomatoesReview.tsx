...
import {Movie, Tomatometer, TomatometerState, useGetRottenTomatoesScoreQuery} from '../../server/generated/graphql';

export default function RottenTomatoesReview({id}: { id: Movie['id'] }) {
  const {data, loading} = useGetRottenTomatoesScoreQuery({variables: {id}})
  ...
  return (
    <div style={{display: 'flex', alignItems: 'center', gap: 20}}>
      <a href={data.movie.tomatometer.link} target="_blank" style={{position: 'relative', color: 'inherit'}}>
        ...
        <Typography variant="h2" sx={{position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', margin: '0'}}>
          {score}%
        </Typography>
      </a>
