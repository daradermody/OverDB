...
import {Tomatometer, TomatometerState} from './generated/graphql';

export default class RottenTomatoes {
  static async getScore(title: string, year: number): Promise<Tomatometer> {
    try {
  ...
  static async getScoreFromPage(link: string): Promise<Tomatometer> {
    const {data} = await axios.get(link)
    const dom = new JSDOM(data)
    const scoreboard = dom.window.document.getElementsByTagName('score-board').item(0)
    const state = stateMap[scoreboard.getAttribute('tomatometerstate')]
    const score = parseInt(scoreboard.getAttribute('tomatometerscore'), 10)
